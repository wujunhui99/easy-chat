...
func (l *GetChatLogLogic) GetChatLog(in *chat.GetChatLogReq) (*chat.GetChatLogResp, error) {
	...
	// 构造查询结果列表
	res := make([]*chat.ChatLog, 0, len(data))
	for _, v := range data {
		res = append(res, &chat.ChatLog{
			Id:             v.ID.Hex(),
