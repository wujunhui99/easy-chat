...
type (
	groupsModel interface {
		...
		ListByCreatorUid(ctx context.Context, creatorUid string) ([]*Groups, error)
...
func (m *defaultGroupsModel) ListByCreatorUid(ctx context.Context, creatorUid string) ([]*Groups, error) {
	query := "select " + groupsRows + " from " + m.tableName() + " where `creator_uid` = ? order by `created_at` desc"
	var resp []*Groups
	err := m.QueryRowsNoCacheCtx(ctx, &resp, query, creatorUid)
	if err != nil {
		return nil, err
	}
	return resp, nil
...
func (m *defaultGroupsModel) Insert(ctx context.Context, session sqlx.Session, data *Groups) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		// created_at / updated_at 由数据库自动填充，不应出现在待插入列列表中，因此 groupsRowsExpectAutoSet 已剔除它们
		// 目前 groupsRowsExpectAutoSet = id,name,icon,status,creator_uid,group_type,is_verify,notification,notification_uid
