...
type groupMsgRead struct {
	...
	push           *msggateway.Push      // 记录推送的消息
	pushCh         chan *msggateway.Push // 最终要推送的
	count          int
	// 上次推送时间
	pushTime time.Time
	done     chan struct{}
...
func newGroupMsgRead(push *msggateway.Push, pushCh chan *msggateway.Push) *groupMsgRead {
	m := &groupMsgRead{
...
// 合并消息
func (m *groupMsgRead) mergePush(push *msggateway.Push) {
	m.mu.Lock()
	defer m.mu.Unlock()
