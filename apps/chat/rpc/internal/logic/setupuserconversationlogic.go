...
// 建立会话: 群聊, 私聊
func (l *SetUpUserConversationLogic) SetUpUserConversation(in *chat.SetUpUserConversationReq) (*chat.SetUpUserConversationResp, error) {
	...
	case constants.SingleChatType: // 私聊
		...
		if err != nil {
			if errors.Is(err, chatmodels.ErrNotFound) {
				// 建立会话
	...
	return &chat.SetUpUserConversationResp{}, nil
}
...
func (l *SetUpUserConversationLogic) setUpUserConversation(conversationId, userId, recvId string,
	...
	if err != nil {
		if errors.Is(err, chatmodels.ErrNotFound) {
			// 如果用户没有会话列表，就创建一个
			conversations = &chatmodels.Conversations{
				ID:               primitive.NewObjectID(),
				UserId:           userId,
				ConversationList: make(map[string]*chatmodels.Conversation),
			}
		} else {
	...
	// 添加会话记录
	conversations.ConversationList[conversationId] = &chatmodels.Conversation{
		ConversationId: conversationId,
		ChatType:       chatType,
		IsShow:         isShow,
	}
