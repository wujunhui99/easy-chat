...
func NewMsgReadTransfer(svc *svc.ServiceContext) kq.ConsumeHandler {
	m := &MsgReadTransfer{
		...
		push:            make(chan *msggateway.Push, 1),
	}
	// 如果开启
...
func (m *MsgReadTransfer) UpdateChatLogRead(ctx context.Context, data *msgtransfer.MsgMarkRead) (map[string]string, error) {
